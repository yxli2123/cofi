...
def prune(module):
    for attr_str in dir(module):
        ...
        if type(target_attr) == LinearLORAS:
            print("====================================================")
    ...
    for attr_str in dir(module):
        ...
        if type(target_attr) == nn.Linear and any(attr_str in an for an in allow_name):
            ...
            if do_svd:
                ...
                linear_loras = LinearLORAS(target_attr.in_features, target_attr.out_features, reduced_rank,
...
if __name__ == '__main__':
    # sparse_weight = torch.randn(12, 8)
    # pruned_idx = [0, 3, 2, 7, 4, 9]
    # sparse_weight[pruned_idx, :] = 0
    #
    # nonzero_idx = torch.nonzero(sparse_weight.sum(dim=1)).flatten()
    # sparse_weight_pruned = sparse_weight[nonzero_idx, :]
    #
    # x = torch.randn(4, 16, 8)  # (B, L, D)
    # B, L, D = x.shape
    # y_ = torch.matmul(x, sparse_weight_pruned.T)
    # y = torch.zeros(B, L, 12)
    # y[..., nonzero_idx] = y_
    # pass
    ...
    model.eval()
    inputs = tokenizer(["Hello, my dog is cute" for _ in range(16)], return_tensors="pt",
    ...
    test_time_mean = np.array(test_time).mean()
    test_time_std = np.array(test_time).std()
    print(test_time_mean, test_time_std)
    # print(outputs1)
    # Decomposition
    allow_name = ['query', 'key', 'value', 'q_proj', 'k_proj', 'v_proj', 'out_proj', 'dense', 'attention', 'fc1', 'fc2']
    block_name = ['pooler', 'classifier', 'LayerNorm', 'embeddings']
    substitute_layer_weights(module=model,
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            if 'sparse' in name and len(param.shape) == 2:
                #print(f"Pruning {name}")
                H, W = param.shape
                zero_idx = random.choices(range(H), k=int(0.9 * H))
                param[zero_idx, :] = 0
    ...
    inputs = tokenizer(["Hello, my dog is very ugly" for _ in range(16)], return_tensors="pt")
    test_time = []
    for _ in range(100):
        start_time = time.time()
        with torch.no_grad():
    ...
    mse = ((outputs1[0] - outputs2[0])**2).mean().sqrt().item()
    print(mse)

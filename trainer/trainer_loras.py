...
class CoFiTrainer(Trainer):
    ...
    def train(self):
        ...
        #                          #
        ############################
        pruner = utils_loras.Pruner(model=model,
                                    args=self.additional_args,
                                    total_step=self.args.max_steps,
                                    mask_param_name=['sparse'],
                                    pruner_name='PLATON',
                                    structured_method=self.additional_args.structured_method,
                                    structured_direction=self.additional_args.structured_direction)
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))): #! 20 epoch
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    threshold, mask_threshold = pruner.update_and_pruning(model, self.global_step)
                    if self.global_step % 1000 == 0:
                        print(f"threshold: {threshold} | mask_threshold: {mask_threshold}")
